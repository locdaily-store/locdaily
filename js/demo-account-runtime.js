(function(){
  ...
  function brandHeader(subtitle,actions=""){return `<header class="dp-production-header"><div class="dp-brand-block"><div class="dp-brand-text"><strong>LocDaily</strong><small>${esc(subtitle)}</small></div></div><div class="dp-header-actions">${actions}</div></header>`;}
  ...
  function renderBarang(state,profile){
    const primaryOwner=profile.id==="owner-pusat";
    const branchOwner=profile.id==="owner-cabang";
    const editable=primaryOwner;
    const stockOperational=profile.role!=="kasir";
    return `<div class="dp-page">
  ...
  function bindStage(state,profile){
    ...
    document.querySelectorAll("[data-demo-delete-product]").forEach(btn=>btn.addEventListener("click",()=>{if(profile.id!=="owner-pusat")return;const i=state.products.findIndex(x=>x.id===btn.dataset.demoDeleteProduct);if(i<0)return;state.products.splice(i,1);persistModeCatalog(state);save(state);render(state);toast("Data item dihapus dari Mode Demo.","warn");}));
    document.getElementById("demoAddProductBtn")?.addEventListener("click",()=>{
      if(profile.id!=="owner-pusat")return;
    ...
    document.getElementById("demoRequestProductBtn")?.addEventListener("click",()=>{
      if(profile.id!=="owner-cabang")return;
      const name=String(document.getElementById("demoRequestName")?.value||"").trim();
      if(name.length<2){toast("Nama item/menu minimal 2 karakter.","warn");return;}
      toast(`Pengajuan ${name} dikirim ke Owner Pusat. Harga beli/HPP tetap tersembunyi dari Owner Cabang.`);
    });
