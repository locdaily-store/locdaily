(function(){
    ...
    function apiReady(){
        return Boolean(window.LDMStockOpname && typeof window.LDMStockOpname.bootstrap === "function");
    }
    ...
    async function boot(trigger="initial"){
        ...
        let failed=false;
        try{
            if(!apiReady()) throw new Error("stock-opname-service.js belum termuat.");

            const result=await window.LDMStockOpname.bootstrap();
            ready=true;
            attempt=0;
            pendingRetryReason="";
            if(retryTimer){
            ...
            window.dispatchEvent(new CustomEvent("ldm-stock-opname-ready",{
            ...
            console.warn("Cloud Stock Opname bootstrap:",error);
            window.dispatchEvent(new CustomEvent("ldm-stock-opname-bootstrap-error",{
    ...
    window.LDMBootstrapState["LDMStockOpname"]={
        version:VERSION,
        isReady:()=>ready,
        isRunning:()=>running,
        attempts:()=>attempt,
