(function(){
    ...
    const SERVICE_VERSION = "27.9.0-v28.18.3";
    ...
    function transactionItemToLegacy(item){
        ...
        const returnedQty = parseNumber(item.approved_return_qty);
        const restockedQty = parseNumber(item.approved_restock_qty);
        const netQty = Math.max(0,parseNumber(item.net_qty != null ? item.net_qty : qty - returnedQty));
        const returnAmount = parseNumber(item.approved_return_amount);
        const effectiveLineTotal = parseNumber(item.effective_line_total || item.line_subtotal);
        const netLineSales = Math.max(0,parseNumber(item.net_line_sales != null ? item.net_line_sales : effectiveLineTotal - returnAmount));
        const grossHpp = parseNumber(item.gross_hpp);
        const reversedHpp = parseNumber(item.reversed_hpp);
        const netHpp = parseNumber(item.net_hpp != null ? item.net_hpp : Math.max(0,grossHpp - reversedHpp));
        ...
        return {
            ...
            originalQty:qty,
            returnedQty,
            approvedReturnQty:returnedQty,
            restockedReturnQty:restockedQty,
            netQty,
            harga,
            hargaAwal:normal,
            hargaNormal:normal,
            ...
            grossSubtotal:effectiveLineTotal,
            returnAmount,
            returAmount:returnAmount,
            netSubtotal:netLineSales,
            netLineSales,
            grossHpp,
            reversedHpp,
            netHpp,
    ...
    function transactionRowToLegacy(row,items){
        ...
        const grossTotal = parseNumber(row.gross_total != null ? row.gross_total : row.grand_total);
        const returnTotal = parseNumber(row.approved_return_total);
        const netTotal = parseNumber(row.net_total != null ? row.net_total : Math.max(0,grossTotal-returnTotal));
        const returnCash = parseNumber(row.approved_return_cash);
        const returnNoncash = parseNumber(row.approved_return_noncash);
        const netCash = parseNumber(row.net_cash_amount != null ? row.net_cash_amount : parseNumber(row.cash_amount)-returnCash);
        const netNoncash = parseNumber(row.net_noncash_amount != null ? row.net_noncash_amount : parseNumber(row.qris_amount)-returnNoncash);
        ...
        return {
            ...
            returTotal:returnTotal,
            approvedReturns:returnTotal,
            netTotal,
            netSales:netTotal,
            ...
            netCash,
            netNoncash,
            grossHpp:parseNumber(row.gross_hpp),
            reversedHpp:parseNumber(row.reversed_hpp),
            netHpp:parseNumber(row.net_hpp),
    ...
    async function fetchTransactionPage(options = {}){
        ...
        const {data,error} = await client().rpc(
            "ldm_report_transactions_page_v28183",
            params
        );
    ...
    async function fetchFinancialSummary(options = {}){
        await ensureAuth();
        const {data,error} = await client().rpc(
            "ldm_report_financial_summary_v28183",
            {
                p_from_date:options.fromDate || null,
                p_to_date:options.toDate || null
            }
        ...
        return {
            fromDate:row.from_date || options.fromDate || "",
            toDate:row.to_date || options.toDate || "",
            grossSales:parseNumber(row.gross_sales),
            approvedReturns:parseNumber(row.approved_returns),
            netSales:parseNumber(row.net_sales),
            cashSales:parseNumber(row.cash_sales),
            noncashSales:parseNumber(row.noncash_sales),
            cashRefunds:parseNumber(row.cash_refunds),
            noncashRefunds:parseNumber(row.noncash_refunds),
            netCash:parseNumber(row.net_cash),
            netNoncash:parseNumber(row.net_noncash),
            transactionCount:Number(row.transaction_count || 0),
            grossHpp:parseNumber(row.gross_hpp),
            reversedHpp:parseNumber(row.reversed_hpp),
            netHpp:parseNumber(row.net_hpp),
            grossProfitBeforeOperating:parseNumber(row.gross_profit_before_operating),
            costVisible:Boolean(row.cost_visible),
            returnBasis:row.return_basis || "approved_at_wita",
            _cloudReturnAware:true
        };
    }

    async function fetchDailyFinancials(options = {}){
        await ensureAuth();
        const {data,error} = await client().rpc(
            "ldm_report_daily_financials_v28183",
            {
                p_from_date:options.fromDate || null,
                p_to_date:options.toDate || null
            }
    ...
    async function refreshProfiles(){
        ...
        if(!storeId){
            throw new Error("Store ID sesi laporan belum tersedia.");
        }
        const supabase = client();
        const {data,error} = await supabase.rpc("ldm_attendance_profiles_v28182",{
            p_store_id:storeId
        });
        if(error){
            throw error;
