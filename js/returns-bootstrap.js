(function(){
    ...
    function apiReady(){
        return Boolean(window.LDMReturns && typeof window.LDMReturns.bootstrap === "function");
    }
    ...
    async function boot(trigger="initial"){
        ...
        let failed=false;
        try{
            if(!apiReady()) throw new Error("returns-service.js belum termuat.");

            const result=await window.LDMReturns.bootstrap();
            ready=true;
            attempt=0;
            pendingRetryReason="";
            if(retryTimer){
            ...
            window.dispatchEvent(new CustomEvent("ldm-returns-ready",{
            ...
            console.warn("Cloud Retur bootstrap:",error);
            window.dispatchEvent(new CustomEvent("ldm-returns-bootstrap-error",{
    ...
    window.LDMBootstrapState["LDMReturns"]={
        version:VERSION,
        isReady:()=>ready,
        isRunning:()=>running,
        attempts:()=>attempt,
