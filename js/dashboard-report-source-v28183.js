(function(){
    ...
    async function cloudSummary(fromDate, toDate){
        if(!window.LDMReporting || typeof window.LDMReporting.fetchFinancialSummary !== "function"){
            throw new Error("Ringkasan keuangan Cloud belum siap.");
        }
        return window.LDMReporting.fetchFinancialSummary({
            fromDate: fromDate || null,
            toDate: toDate || null
        });
    }

    async function cloudDaily(fromDate, toDate){
        if(!window.LDMReporting || typeof window.LDMReporting.fetchDailyFinancials !== "function"){
            return [];
        }
        return window.LDMReporting.fetchDailyFinancials({
            fromDate: fromDate || null,
            toDate: toDate || null
        });
    }
    ...
    function deriveSummary(rows){
        ...
        return list.reduce((acc,row)=>{
            const gross=Number(row.grossTotal ?? row.total ?? row.grandTotal ?? 0) || 0;
            const returned=Number(row.returTotal ?? row.approvedReturns ?? 0) || 0;
            const net=Number(row.netTotal ?? row.netSales ?? Math.max(0,gross-returned)) || 0;
            acc.grossSales+=gross;
            acc.approvedReturns+=returned;
            acc.netSales+=net;
            acc.netCash+=Number(row.netCash ?? row.cash ?? row.bayarCash ?? 0) || 0;
            acc.netNoncash+=Number(row.netNoncash ?? row.qris ?? row.bayarQris ?? 0) || 0;
            acc.netHpp+=Number(row.netHpp ?? 0) || 0;
    ...
    async function loadRange(options = {}){
        ...
        if(preferCloud && navigator.onLine !== false){
            try{
                const [rows,summary,daily] = await Promise.all([
                    cloudRows(fromDate, toDate),
                    cloudSummary(fromDate, toDate),
                    cloudDaily(fromDate, toDate)
                ]);
                await mirrorToIndexedDB(rows);
                return { rows, summary, daily, source: "cloud", fromDate, toDate };
            }catch(error){
                cloudError = error;
        ...
        try{
            ...
            if(rows.length || navigator.onLine === false){
                return { rows, summary:deriveSummary(rows), daily:[], source: "indexeddb", fromDate, toDate, cloudError };
            }
        }catch(error){
            console.warn("[LocDaily] Arsip lokal utama gagal, memakai cache browser sementara:", error);
        }

        const rows=localRows(fromDate, toDate);
        return {
            rows,
