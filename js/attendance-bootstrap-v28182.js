(function(){
    ...
    function apiReady(){
        return Boolean(window.LDMAttendance && typeof window.LDMAttendance.bootstrap === "function");
    }
    ...
    async function boot(trigger="initial"){
        ...
        let failed=false;
        try{
            if(!apiReady()) throw new Error("attendance-service.js belum termuat.");

            const result=await window.LDMAttendance.bootstrap();
            ready=true;
            attempt=0;
            pendingRetryReason="";
            if(retryTimer){
            ...
            window.dispatchEvent(new CustomEvent("ldm-attendance-ready",{
            ...
            console.warn("Cloud Attendance bootstrap:",error);
            window.dispatchEvent(new CustomEvent("ldm-attendance-bootstrap-error",{
    ...
    window.LDMBootstrapState["LDMAttendance"]={
        version:VERSION,
        isReady:()=>ready,
        isRunning:()=>running,
        attempts:()=>attempt,
