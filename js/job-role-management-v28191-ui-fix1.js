(function(){
    ...
    function renderPermissions(){
        ...
        container.innerHTML=groups.map(group=>{
            ...
            return `<section class="jr-permission-group"><h3>${esc(publicText(group))}</h3><div class="jr-permission-list">${items.map(item=>{
                const baseAllowed=permissionAllowedForBase(item,draft.base_system_role);
                ...
                let badge='';
                if(protectedItem)badge='<span class="jr-pill warn">Pemilik Utama</span>';
                else if(mandatory)badge='<span class="jr-pill green">Diperlukan</span>';
                else if(!baseAllowed)badge='<span class="jr-pill">Tidak tersedia</span>';
                return `<label class="jr-permission ${cls}"><input type="checkbox" data-permission="${esc(item.code)}" ${checked?'checked':''} ${disabled?'disabled':''}><span><strong>${esc(publicPermissionLabel(item))} ${badge}</strong><small>${esc(publicPermissionDescription(item))}</small></span></label>`;
            }).join("")}</div></section>`;
        }).join("");
    ...
    function renderImpact(){
        ...
        $("jrPermissionCountText").textContent=`${selected + mandatory} fitur tersedia`;
    }
