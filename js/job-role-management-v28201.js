(function(){
    ...
    const state={catalog:[],roles:[],stores:[],assignments:[],audit:[],selectedId:null,editing:null,context:null,permissionSearch:"",permissionFilter:"all"};
    ...
    function permissionDefaultEnabled(item,base){
        return item?.mandatory===true || (Array.isArray(item?.default_enabled_roles)&&item.default_enabled_roles.includes(base));
    }
    function refreshPermissionEditor(){
        renderSensitivePermissions();
        renderPermissions();
        renderLimitSettings();
        renderImpact();
    }
    function applyModuleAdvancedPreset(parentCode,preset){
        const draft=editorDraft();
        const parent=state.catalog.find(item=>item.code===parentCode);
        if(!parent||!permissionAllowedForBase(parent,draft.base_system_role))return;
        const children=moduleChildren(parentCode);
        const set=new Set(draft.permissions||[]);
        if(preset==="all"){
            set.add(parentCode);
            children.forEach(item=>set.add(item.code));
        }else if(preset==="standard"){
            children.forEach(item=>set.delete(item.code));
            if(permissionDefaultEnabled(parent,draft.base_system_role))set.add(parentCode);else if(!parent.mandatory)set.delete(parentCode);
            children.forEach(item=>{
                if(permissionDefaultEnabled(item,draft.base_system_role)){
                    set.add(parentCode);
                    set.add(item.code);
                }
            });
        }else if(preset==="clear-actions"){
            children.forEach(item=>{
                if(item.mandatory)set.add(item.code);else set.delete(item.code);
            });
            if(children.some(item=>item.mandatory))set.add(parentCode);
        }
        draft.permissions=[...set];
        hideDropdown();
        refreshPermissionEditor();
    }
    function advancedDropdownMarkup(parentCode){
        const draft=editorDraft();
        const parent=state.catalog.find(item=>item.code===parentCode);
        if(!parent)return "";
        const children=moduleChildren(parentCode);
        const activeChildren=children.filter(item=>hasPermission(item.code)||item.mandatory===true);
        const importantChildren=children.filter(item=>item.risk_level==="sensitive"||item.risk_level==="critical");
        const activeImportant=importantChildren.filter(item=>hasPermission(item.code)||item.mandatory===true);
        const roleLabel=ROLE_LABELS[draft.base_system_role]||"akun ini";
        const discountItem=children.find(item=>item.code==="pos.discount.apply");
        ...
        return `<div class="jr-advanced-dropdown-card" role="dialog" aria-modal="false" aria-label="Pengaturan lanjutan ${esc(publicPermissionLabel(parent))}">
          <div class="jr-advanced-dropdown-head">
            <div><span class="jr-advanced-kicker">PENGATURAN LANJUTAN</span><strong>${icon} ${esc(publicPermissionLabel(parent))}</strong><small>${esc(publicPermissionDescription(parent))}</small></div>
            <button type="button" class="jr-advanced-close" data-advanced-close aria-label="Tutup pengaturan lanjutan">×</button>
          </div>
          <div class="jr-advanced-summary">
            <span><b>${activeChildren.length}</b> dari ${children.length} tindakan aktif</span>
            <span><b>${activeImportant.length}</b> tindakan penting aktif</span>
          </div>
          <div class="jr-advanced-section">
            <strong>Pengaturan cepat</strong>
            <small>Gunakan pilihan ini untuk mengatur seluruh tindakan pada halaman ini tanpa memilih satu per satu.</small>
            <div class="jr-advanced-buttons">
              <button type="button" data-advanced-preset="all">Aktifkan semua tindakan</button>
              <button type="button" data-advanced-preset="standard">Gunakan akses standar ${esc(roleLabel)}</button>
              <button type="button" data-advanced-preset="clear-actions">Matikan tindakan tambahan</button>
            </div>
          </div>
          ${discountItem?`<div class="jr-advanced-section">
            <strong>Batas diskon</strong>
            <small>Batasi diskon manual maksimum untuk jabatan ini. Pengaturan dapat diubah jika tindakan Beri Diskon aktif.</small>
            <label class="jr-advanced-limit ${discountEnabled?'':'disabled'}"><span>Maksimum diskon per transaksi</span><span><input type="number" min="0" max="100" step="0.01" inputmode="decimal" data-advanced-discount value="${esc(discountLimit)}" ${discountEnabled?'':'disabled'}><b>%</b></span></label>
          </div>`:""}
          ${importantChildren.length?`<div class="jr-advanced-section">
            <strong>Tindakan penting</strong>
            <small>Periksa kembali izin ini karena dapat memengaruhi uang, stok, atau data operasional.</small>
            <div class="jr-advanced-important">${importantChildren.map(item=>`<span class="${hasPermission(item.code)||item.mandatory===true?'active':''}">${esc(publicPermissionLabel(item))}</span>`).join("")}</div>
          </div>`:""}
        </div>`;
    }
    function positionDropdown(dropdown,anchor){
        ...
        if(window.matchMedia("(max-width:700px)").matches){
            dropdown.style.left="";dropdown.style.top="";dropdown.style.width="";
            return;
        }
        const rect=anchor.getBoundingClientRect();
        const width=Math.min(390,Math.max(300,window.innerWidth-24));
        dropdown.style.width=`${width}px`;
        dropdown.style.left=`${Math.max(12,Math.min(window.innerWidth-width-12,rect.right-width))}px`;
        dropdown.style.top="12px";
        const height=Math.min(dropdown.scrollHeight||520,window.innerHeight-24);
        const below=window.innerHeight-rect.bottom-10;
        const top=below>=Math.min(height,420)?rect.bottom+8:Math.max(12,rect.top-height-8);
        dropdown.style.top=`${top}px`;
    }
    function hideDropdown(){
        const dropdown=$("jrAdvancedDropdown");
        if(dropdown){dropdown.classList.remove("show");dropdown.setAttribute("aria-hidden","true");dropdown.innerHTML="";}
        document.querySelectorAll("[data-advanced-module][aria-expanded='true']").forEach(button=>button.setAttribute("aria-expanded","false"));
    }
    function showDropdown(anchor,parentCode){
        if(!anchor||!parentCode)return;
        let dropdown=$("jrAdvancedDropdown");
        if(dropdown?.classList.contains("show")&&dropdown.dataset.parentCode===parentCode&&anchor.getAttribute("aria-expanded")==="true"){hideDropdown();return;}
        if(!dropdown){
            dropdown=document.createElement("div");
            dropdown.id="jrAdvancedDropdown";
            dropdown.className="jr-advanced-dropdown";
            dropdown.setAttribute("aria-hidden","true");
            document.body.appendChild(dropdown);
        }
        document.querySelectorAll("[data-advanced-module][aria-expanded='true']").forEach(button=>button.setAttribute("aria-expanded","false"));
        anchor.setAttribute("aria-expanded","true");
        dropdown.dataset.parentCode=parentCode;
        dropdown.innerHTML=advancedDropdownMarkup(parentCode);
        dropdown.classList.add("show");
        dropdown.setAttribute("aria-hidden","false");
        positionDropdown(dropdown,anchor);
        dropdown.querySelector("[data-advanced-close]")?.addEventListener("click",hideDropdown);
        dropdown.querySelectorAll("[data-advanced-preset]").forEach(button=>button.addEventListener("click",()=>applyModuleAdvancedPreset(parentCode,button.dataset.advancedPreset)));
        dropdown.querySelector("[data-advanced-discount]")?.addEventListener("input",event=>{
    ...
    function renderPermissions(){
        ...
        container.innerHTML=modules.map(({parent,children,parentSelected,anySelected})=>{
            ...
            return `<section class="jr-access-module ${anySelected?'selected':''}">
              ...
              </label><div class="jr-access-module-tools"><span class="jr-pill">${activeCount}/${allChildren.length} tindakan</span>${allChildren.length?`<button type="button" class="jr-advanced-trigger" data-advanced-module="${esc(parent.code)}" aria-haspopup="dialog" aria-expanded="false">⋮ Lanjutan</button>`:""}</div></div>
              ${children.length?`<div class="jr-access-actions">${children.map(child=>{
        ...
        container.querySelectorAll("[data-advanced-module]").forEach(button=>button.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();showDropdown(button,button.dataset.advancedModule);}));
    }
    ...
    function renderAssignments(){
        ...
        if(!state.assignments.length){tbody.innerHTML='<tr><td colspan="7" class="jr-empty">Belum ada karyawan yang dapat ditampilkan.</td></tr>';return;}
    ...
    document.addEventListener("click",event=>{
        ...
        if(dropdown.contains(event.target)||event.target.closest?.("[data-advanced-module]"))return;
        hideDropdown();
    });
