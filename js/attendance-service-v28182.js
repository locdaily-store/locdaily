(function(){
    ...
    async function fetchProfiles(){
        ...
        if(!storeId){
            throw new Error("Store ID pada sesi Cloud Absensi belum tersedia.");
        }
        ...
        const {
            ...
            "ldm_attendance_profiles_v28182",
            {p_store_id:storeId}
        );
    ...
    async function recordAttendance({
        ...
        const target = profiles.find(
            profile => normalizeUsername(profile.username) === wanted
        );
        ...
        if(role !== "owner" && target.id !== context?.user?.id){
            throw new Error(
        ...
        try{
            ...
            throw new Error(`Upload bukti presensi gagal: ${message}`);
        }
        ...
        try{
            const result = await supabase.rpc(
                "ldm_record_attendance_v28182",
                {
                    p_store_id:storeId,
                    p_target_user_id:target.id,
                    p_attendance_type:type,
                    p_shift_label:shift || null,
                    p_note:note || null,
            ...
            const message=error?.message || String(error);
            throw new Error(`Penyimpanan presensi gagal: ${message}`);
        }

        // Setelah RPC berhasil, presensi SUDAH tersimpan.
        // Refresh cache tidak boleh mengubah status sukses menjadi gagal.
        ...
        try{
            ...
            console.warn(
                "Presensi tersimpan, tetapi refresh cache tertunda:",
                error
            );
            window.dispatchEvent(new CustomEvent("ldm-attendance-cache-refresh-warning",{
