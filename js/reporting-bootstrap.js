(function(){
    ...
    function apiReady(){
        return Boolean(window.LDMReporting && typeof window.LDMReporting.bootstrap === "function");
    }
    ...
    async function boot(trigger="initial"){
        ...
        let failed=false;
        try{
            if(!apiReady()) throw new Error("reporting-service.js belum termuat.");
            ...
            window.dispatchEvent(new CustomEvent("ldm-reporting-ready",{
            ...
            window.dispatchEvent(new CustomEvent("ldm-reporting-bootstrap-error",{
    ...
    window.LDMBootstrapState["LDMReporting"]={
        version:VERSION,
        isReady:()=>ready,
        isRunning:()=>running,
        attempts:()=>attempt,
