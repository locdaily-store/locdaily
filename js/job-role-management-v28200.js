(function(){
    ...
    function renderPermissions(){
        ...
        pages.forEach(parent=>{
            ...
            let children=state.catalog.filter(item=>item.active!==false&&!item.primary_owner_only&&item.parent_permission_code===parent.code&&permissionAllowedForBase(item,draft.base_system_role));
        ...
        container.innerHTML=modules.map(({parent,children,parentSelected,anySelected})=>{
            ...
            return `<section class="jr-access-module ${anySelected?'selected':''}">
              ...
              </label><span class="jr-pill">${children.length} tindakan</span></div>
        ...
        container.querySelectorAll("[data-page-permission]").forEach(input=>input.addEventListener("change",()=>{setPermission(input.dataset.pagePermission,input.checked);renderPermissions();renderSensitivePermissions();renderLimitSettings();renderImpact();}));
        container.querySelectorAll("[data-action-permission]").forEach(input=>input.addEventListener("change",()=>{setPermission(input.dataset.actionPermission,input.checked);renderPermissions();renderLimitSettings();renderImpact();}));
    }
