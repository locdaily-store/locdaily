(function(){
    ...
    function permissionDeniedMessage(permissionContext,page){
        const jobName=permissionContext?.job_role_name||"Jabatan akun ini";
        if(permissionContext?.assignment_mode==="custom"&&permissionContext?.job_role_active===false){
            return `${jobName} sedang dinonaktifkan. Hubungi Owner Pusat untuk memperbarui jabatan.`;
        }
        if(permissionContext?.assignment_mode==="custom"&&permissionContext?.scope_valid===false){
            return `${jobName} tidak berlaku untuk cabang akun ini. Hubungi Owner Pusat untuk menyesuaikan jabatan.`;
        }
        return `${jobName} tidak memiliki hak akses untuk membuka ${accessLabel(page)}.`;
    }
    function permissionAllowsPage(permissionContext,page=currentPage()){
        const policy=policyFor(page);
        if(!policy?.permission)return true;
        if(permissionContext?.assignment_mode==="custom"&&(permissionContext?.job_role_active===false||permissionContext?.scope_valid===false))return false;
        return Array.isArray(permissionContext?.permissions)&&permissionContext.permissions.includes(policy.permission);
    }
    let authorizedOnce=false;
    function recheckPermissionContext(permissionContext){
        if(!authorizedOnce||window.__LDM_ROLE_ACCESS_REDIRECTING)return;
        const page=currentPage();
        const policy=policyFor(page);
        if(!policy?.permission)return;
        if(!permissionAllowsPage(permissionContext,page)){
            redirectDenied(permissionDeniedMessage(permissionContext,page));
            return;
        }
        window.LDM_EMPLOYEE_PERMISSION_CONTEXT=permissionContext||null;
    ...
    async function authorize(context){
        ...
        if(policy.permission){
            ...
            try{
                permissionContext=await window.LDMEmployeePermissions.context(true,context||window.LDM_CLOUD_CONTEXT||null);
            }catch(error){
    ...
    window.addEventListener("ldm-employee-permissions-ready",event=>{
        if(event?.detail)recheckPermissionContext(event.detail);
    });
    window.addEventListener("ldm-employee-permissions-changed",event=>{
        if(event?.detail)recheckPermissionContext(event.detail);
    });
    ...
    window.LDMRoleAccess=Object.freeze({
        ...
        recheck:()=>recheckPermissionContext(window.LDMEmployeePermissions?.current?.()||null)
    });
})();
